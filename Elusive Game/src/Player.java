...
public class Player {
	...
	//private boolean fireResistance; ***IF ROYAL KNIGHT ARMOR IS EQUIPPED***
	...
	public void exploreRoom() {
		...
		if(currentRoom.getItem().size() == 0) {
			...
			for(Items i : currentRoom.getItem() ){
				System.out.println(i.getItemName());
			}
		}
		
		if(currentRoom.getNpc() != null) {
			System.out.println("NPC in room: " + currentRoom.getNpc().getNpcName());
		}
	...
	public void talk(String name) {
		System.out.println();
		if(name.equalsIgnoreCase((currentRoom.getNpc().getNpcName()))) {
			currentRoom.getNpc().talkTo(this);
		}
		else {
			System.out.println("There is no NPC with that name here.");
		}
	}
